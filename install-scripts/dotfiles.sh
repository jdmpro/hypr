...
printf "${NOTE} Downloading / Checking for existing hyprdot.tar.gz...\n"

# Check if Hyprland-Dots.tar.gz exists
if [ -f hyprdot.tar.gz ]; then
  printf "${NOTE} hyprdot.tar.gz found.\n"

  # Get the version from the existing tarball filename
  existing_version=$(echo hyprdot.tar.gz | grep -oP 'v\d+\.\d+\.\d+' | sed 's/v//')

  # Fetch the tag_name for the latest release using the GitHub API
  latest_version=$(curl -s https://api.github.com/repos/jdmpro/hyprdot/releases/latest | grep "tag_name" | cut -d '"' -f 4 | sed 's/v//')

  # Check if versions match
  if [ "$existing_version" = "$latest_version" ]; then
    echo -e "${OK} hyprdot.tar.gz is up-to-date with the latest release ($latest_version)."
    ...
    echo -e "${WARN} hyprdot.tar.gz is outdated (Existing version: $existing_version, Latest version: $latest_version)."
    ...
    if [ "$upgrade_choice" = "y" ]; then
      ...
      find . -type d -name 'jdmpro-hyprdot*' -exec rm -rf {} +
      rm -f hyprdot.tar.gz
      printf "${WARN} Removed existing hyprdot.tar.gz.\n"
    else
...
latest_tag=$(curl -s https://api.github.com/repos/jdmpro/hyprdot/releases/latest | grep "tag_name" | cut -d '"' -f 4)
...
latest_tarball_url=$(curl -s https://api.github.com/repos/jdmpro/hyprdot/releases/latest | grep "tarball_url" | cut -d '"' -f 4)
...
file_name="hyprdot-${latest_tag}.tar.gz"
...
if curl -L "$latest_tarball_url" -o "$file_name"; then
  ...
  rm -rf jdmpro-hyprdot

  # Identify the extracted directory
  extracted_directory=$(tar -tf "$file_name" | grep -o '^[^/]\+' | uniq)

  # Rename the extracted directory to jdmpro-hyprdot
  mv "$extracted_directory" jdmpro-hyprdot || exit 1

  cd "jdmpro-hyprdot" || exit 1

  # Set execute permission for copy.sh and execute it
  chmod +x copy.sh
  ./copy.sh 

  echo -e "${OK} Latest Dotfiles release downloaded, extracted, and processed successfully. Check jdmpro-hyprdot folder for more detailed install logs" 2>&1 | tee -a "../Install-Logs/install-$(date +'%d-%H%M%S')_dotfiles.log"
else
